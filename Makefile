...
install: create-cloud-native-demo create-kubecon-demo create-toggle-shop
...
create-toggle-shop:
	sed 's~{{GITOPS_REPO}}~$(GITOPS_REPO)~g' toggle-shop/app.yaml > toggle-shop/app_tmp.yaml
	kubectl -n argocd apply -f toggle-shop/app_tmp.yaml
	rm toggle-shop/app_tmp.yaml
